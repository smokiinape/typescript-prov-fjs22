// UPPGIFT: Definiera en index signature för Dictionary-typen samt implementera funktionerna addWord och getDefinition. / 3 poäng
...
type Dictionary = {
  ...
  function addWord(dictionary: Dictionary, word: string, definition: string): void {
    // Implementera frn här funktionen så att den lägger till ordet och definitionen i ordboken (dictionary).
   
  }
  

  function getDefinition(dictionary: Dictionary, word: string): string | undefined {
    // Implementera frn här funktionen så att den returnerar definitionen av ordet (word) om det finns i ordboken (dictionary).
  ...
  console.log(getDefinition(myDictionary, "JavaScript")); // Undefinied
